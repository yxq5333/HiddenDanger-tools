...
public class DangerProductModel implements DangerProductContract.Model {

    private DangerProductContract.View mView;

    public DangerProductModel(DangerProductContract.View mView) {
        this.mView = mView;
    }
    ...
    public void getData(Map<String, Object> request) {
        ...
        RequestParams params = new RequestParams(ZIPConfigHelper.get().getNowIPConfig().getRequestUrl(InterfaceApi.DangerProductList.getUrl()));
            ...
            public void onGetSuccess(String result) {
                ...
                if (isViewAttached()) {
                    if (responseInfo.getCode() == ResponseInfo.CODE_SUCCESS) {
                        new Thread(() -> {
                            ArrayList<DangerProduct> tempData = new Gson().fromJson(
                                    new Gson().toJson(responseInfo.getPage().getList()),
                                    new TypeToken<ArrayList<DangerProduct>>() {
                                    }.getType()
                            );
