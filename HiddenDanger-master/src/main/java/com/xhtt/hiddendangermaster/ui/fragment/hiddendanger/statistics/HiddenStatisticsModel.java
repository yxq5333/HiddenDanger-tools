...
public class HiddenStatisticsModel implements HiddenStatisticsContract.Model {

    private HiddenStatisticsContract.View mView;

    public HiddenStatisticsModel(HiddenStatisticsContract.View mView) {
        this.mView = mView;
    }
    ...
    public void getBaseData(Map<String, Object> request) {
        ...
        RequestParams params = new RequestParams(IPConfigHelper.create().getNowIPConfig().getRequestUrl(InterfaceApi.HiddenDangerStatisticsBase.getUrl()));
            ...
            public void onGetSuccess(String result) {
                ...
                if (isViewAttached()) {
                    if (responseInfo.getCode() == ResponseInfo.CODE_SUCCESS) {
                        new Thread(() -> {
                            HiddenStatisticsBase tempData = new Gson().fromJson(
                                    new Gson().toJson(responseInfo.getData()),
                                    HiddenStatisticsBase.class
                            );
    ...
    public void getListData(Map<String, Object> request) {
        ...
        RequestParams params = new RequestParams(IPConfigHelper.create().getNowIPConfig().getRequestUrl(InterfaceApi.HiddenDangerStatisticsList.getUrl()));
        params.setMethod(HttpMethod.GET);
        params.addHeader("token", MyApplication.createApplication().getToken());
        ...
        new XUtils2.BuilderGetHttpData().setGetHttpDataListener(new GetHttpDataListener() {
            ...
            public void onGetSuccess(String result) {
                ...
                if (isViewAttached()) {
                    if (responseInfo.getCode() == ResponseInfo.CODE_SUCCESS) {
                        new Thread(() -> {
                            ArrayList<HiddenStatistics> tempData = new Gson().fromJson(
                                    new Gson().toJson(responseInfo.getPage().getList()),
                                    new TypeToken<ArrayList<HiddenStatistics>>() {
                                    }.getType()
                            );
