...
public class MSDSModel implements MSDSContract.Model {

    private MSDSContract.View mView;

    public MSDSModel(MSDSContract.View mView) {
        this.mView = mView;
    }
    ...
    public void getData(Map<String, Object> request) {
        ...
        RequestParams params = new RequestParams(IPConfigHelper.create().getNowIPConfig().getRequestUrl(InterfaceApi.MSDSList.getUrl()));
            ...
            public void onGetSuccess(String result) {
                ...
                if (isViewAttached()) {
                    if (responseInfo.getCode() == ResponseInfo.CODE_SUCCESS) {
                        new Thread(() -> {
                            ArrayList<MSDS> tempData = new Gson().fromJson(
                                    new Gson().toJson(responseInfo.getPage().getList()),
                                    new TypeToken<ArrayList<MSDS>>() {
                                    }.getType()
                            );
