...
public class TechnologyStandardModel implements TechnologyStandardContract.Model {

    private TechnologyStandardContract.View mView;

    public TechnologyStandardModel(TechnologyStandardContract.View mView) {
        this.mView = mView;
    }
    ...
    public void getData(Map<String, Object> request) {
        ...
        RequestParams params = new RequestParams(IPConfigHelper.create().getNowIPConfig().getRequestUrl(InterfaceApi.TechnologyStandardList.getUrl()));
            ...
            public void onGetSuccess(String result) {
                ...
                if (isViewAttached()) {
                    if (responseInfo.getCode() == ResponseInfo.CODE_SUCCESS) {
                        new Thread(() -> {
                            ArrayList<TechnologyStandard> tempData = new Gson().fromJson(
                                    new Gson().toJson(responseInfo.getPage().getList()),
                                    new TypeToken<ArrayList<TechnologyStandard>>() {
                                    }.getType()
                            );
